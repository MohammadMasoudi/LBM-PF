...
# this case is made based on: 
# https://docs.tclb.io/tutorials/additional/2.-multiphase-rti/
...
At= 0.5 #(Density_real_H -Density_real_L)/(Density_real_H +Density_real_L)    # Atwood number
Re= double(3000)
Ca= 0.26
Pe= double(100)
t0=double(1000)#sqrt(L/abs(g_b_y)/At)
...
surface_tension = 5e-5#visc_H*U0/Ca #surface_tension_real * C_t**2 / (C_rho) / (C_L**3) 
mobility = 0.01#U0*L/Pe#double(0.0082)                               # eq.(11) - Mobility = thau_phi*Cs2*dt # --> this value is proposed in text Sect. 3.2 and Fakhari et al. (2017a)


n_steps = 6*t0.astype(int) 
...
for i in range(nx):
    for j in range(ny):
        InterfacePoint = MidPoint + 0.1*L*cos(2.0*pi*i/L) #eq.39 Ref.2
        phi[j,i] = PhaseField_av + 0.5 * (PhaseField_h - PhaseField_l) * tanh(2.0 * ((ny-1-j) - InterfacePoint)/interface_thickness)#eq 41. ref2
...
for t in range(n_steps+10):
    ...
    ## phase field:
    phi = sum(h, axis=2)
    phi[phi<PhaseField_l]= PhaseField_l
    phi[phi>PhaseField_h]= PhaseField_h
    #B.C:
    ...
    if (mod(t,100)==0):
        ...
        results['density']=rho
        results['Pressure']=P
        fname_pkl='RESULTS/P2/results_%s.pkl' % (t/100)

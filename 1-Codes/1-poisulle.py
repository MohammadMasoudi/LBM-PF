...
from numpy import identity, logical_and,array, ones, zeros, empty, diag, sum, sqrt, cos, pi, mean, mod, double, matmul
from math import tanh
from tkinter import filedialog
from pandas import read_excel, DataFrame, ExcelWriter
from AdditionalFunctions import normal_vector, BounceBackD2Q9, phi_1st_2nd_derivative, phi_1st_2nd_SimplewithSolid, phi_1st_2nd_isotropic
...
visc_real_L = double(0.00005)                   # kg/(m.s)
...
#    phi[logical_and (struct==0,(coords_solid_boundaries[:,:,0]==-100)) ]=0
    ...
    # =============================================================================
        ...
        u_aparent = rho_u/rho 
        u1 = u_aparent/Cs2 + dt*(Fs_x+Fb_x)/(2*rho)     # eq.(21a)
    ...
    visc = visc_L + phi*(visc_H - visc_L)
    
    ################################################################
    # relaxation time --> eq.(20)
    tau_rho = visc / (rho*Cs2)
    ################################################################
    Sv = 1/(tau_rho + 0.5)     # from Fakhari and Bolster 2017 - eq.(23)
